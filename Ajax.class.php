...
function getPrecioPromedioCodigo() {
    $codigo = $_POST['codigo'];
    $AvgPrice = getResultArrayMSSQL("SELECT ItemCode,AvgPrice From  OITM WHERE ItemCode = '$codigo' ");
...
function getDescuentosYSubtotales($factura){    
    $sql = "SELECT lote,descuento,subtotal FROM fact_vent_det WHERE f_nro = $factura";
...
function cerrarVenta() {
    ...
    if ($control == "Ok") {
        if ($vuelto_gs > 0) {
            $db->Query("INSERT INTO efectivo(f_nro, m_cod, entrada,salida, cotiz,entrada_ref,salida_ref,fecha,hora,id_concepto,suc,estado,fecha_reg,usuario)VALUES ('$factura','$moneda_vuelto',0,$vuelto, $cotiz,0,$vuelto_gs,current_date,current_time,2,'$suc','Pendiente',current_date,'$usuario');");
        }
